...
def face_value(value):
    ...
    if not (MIN_FACE_VALUE <= value <= MAX_FACE_VALUE):
        raise ValueError('face value must be between {} and {} inclusive'.format(
            MIN_FACE_VALUE, MAX_FACE_VALUE))
...
def suit_value(value):
    ...
    if not (MIN_SUIT_VALUE <= value <= MAX_SUIT_VALUE):
        raise ValueError('the suit value must be at least {} and at most {}'.format(
            MIN_SUIT_VALUE, MAX_SUIT_VALUE))
...
def num_players(value):
    ...
    if not MIN_PLAYERS <= value <= MAX_PLAYERS:
        raise ValueError('the number of players must be at least {} and at most {}'.format(
